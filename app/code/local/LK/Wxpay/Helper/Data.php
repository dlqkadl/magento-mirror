...
class LK_Wxpay_Helper_Data extends Mage_Core_Helper_Abstract
{
    const API_BASE_URL = "https://pay.weixin.qq.com/wiki/doc/api";
    const API_PARAMS = "";
...
//         }
    ...
    public function arrayToXml_getsignkey($arr)
    {
        $xml = "<xml>";
        foreach ($arr as $key => $val) {
            if ($key == 'mch_id' or $key == 'nonce_str' or $key == 'sign'){
                if (is_numeric($val)) {
                    $xml .= "<" . $key . ">" . $val . "</" . $key . ">";
                } else
                    $xml .= "<" . $key . "><![CDATA[" . $val . "]]></" . $key . ">";
